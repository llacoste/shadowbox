...
def process(
    ...
    notes: list[str] = []
    kerf_radius_px = _kerf_radius_px(settings.kerf_mm, width_px, settings.width_mm)
    if kerf_radius_px > 0:
    ...
    min_feature_pixels = max(
        0,
        round(_mm_to_px(settings.min_feature_mm, width_px, settings.width_mm)),
    )
    ...
    for i, paths in enumerate(per_layer_paths, start=1):
        svg = compose.compose_layer(
            ...
            canvas_mm=(settings.width_mm, settings.height_mm),
            layer_index=i,
